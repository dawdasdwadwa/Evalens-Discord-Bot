...
Внутри тикета есть четыре кнопки (все — только для стаффа):
...
- «Добавить пользователя» — открывает выбор участника и даёт ему доступ на чтение
  канала (видит переписку, писать не может).
...
class TicketActionsView(ui.View):
    ...
    async def add_user(self, interaction: discord.Interaction, button: ui.Button):
        if not _is_staff(interaction.user):
            await interaction.response.send_message("Добавлять пользователей может только стафф.", ephemeral=True)
            return
    ...
    async def delete_ticket(self, interaction: discord.Interaction, button: ui.Button):
        if not _is_staff(interaction.user):
            await interaction.response.send_message("Удалить тикет может только стафф.", ephemeral=True)
            return
