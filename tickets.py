...
(своя для «Жалобы на игрока» и своя для «Жалобы на персонал»); при закрытии канал не
удаляется, а переносится в соответствующую категорию для закрытых тикетов. Открытие и
закрытие тикетов дублируется в settings.TICKET_LOG_CHANNEL_ID.
"""
...
class TicketCloseView(ui.View):
    ...
    async def close_ticket(self, interaction: discord.Interaction, button: ui.Button):
        ...
        # закрываем автору тикета доступ к отправке сообщений (архив только для чтения)
        try:
            if owner_id:
        ...
        # отключаем кнопку "Закрыть тикет" в исходном сообщении
...
class Tickets(commands.Cog):
    ...
    async def log_ticket_event(self, guild: discord.Guild, *, title: str, description: str, color: discord.Color):
        ...
        *,
        ...
        try:
            channel = await guild.create_text_channel(
                ...
                topic=str(interaction.user.id),  # используется при закрытии, чтобы найти автора
