"""
Ког верификации участников по кнопке (аналогично боту Talent).

Логика:
- Администратор один раз вызывает команду /setup_verification
  в канале верификации — бот публикует embed с кнопкой.
- Кнопка постоянная (persistent view с custom_id), поэтому продолжает
  работать после перезапуска бота.
- При нажатии пользователю выдаётся роль VERIFIED_ROLE_ID
  (и, если задано, снимается UNVERIFIED_ROLE_ID).
"""
...
class Verification(commands.Cog):
    def __init__(self, bot: commands.Bot):
        self.bot = bot
        # регистрируем persistent view сразу, чтобы кнопка работала
        # даже если сообщение с ней было отправлено до перезапуска бота
